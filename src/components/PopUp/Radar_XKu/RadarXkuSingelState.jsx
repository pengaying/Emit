...
export default class RadarXkuSingelState extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    ...
    let lights = ['light1', 'light2', 'light3', 'light4', 'light5', 'light6', 'light7', 'light8'];
    for (let i = 0; i < lights.length; i++)
      lights[i] = whiteLight;
    if (this.props.socketio.data != null && this.props.socketio.data.length > 0) {
      if (this.props.socketio.data[1].workingCondition === 1) {  //工作状态
        lights[0] = redLight;
      }
      else if (this.props.socketio.data[1].workingCondition === 0) {
        lights[0] = greenLight;
      }
      // if (this.props.socketio.data[1].inputIncentiveInstructions === 1) {//输入激励指示
      //   lights[1] = redLight;
      // }
      // else if (this.props.socketio.data[1].inputIncentiveInstructions === 0) {
      //   lights[1] = greenLight;
      // }
      if (this.props.socketio.data[1].outputPowerIndication === 1) {//输出功率
        lights[2] = redLight;
      }
      else if (this.props.socketio.data[1].outputPowerIndication === 0) {
        lights[2] = greenLight;
      }
      if (this.props.socketio.data[1].overCurrent === 1) { //过流
        lights[3] = redLight;
      }
      else if (this.props.socketio.data[1].overCurrent === 0) {
        lights[3] = greenLight;
      }
      if (this.props.socketio.data[1].overvoltage === 1) {//过压
        lights[4] = redLight;
      }
      else if (this.props.socketio.data[1].overvoltage === 0) {
        lights[4] = greenLight;
      }
      if (this.props.socketio.data[1].overheated === 1) {//过热
        lights[5] = redLight;
      }
      else if (this.props.socketio.data[1].overheated === 0) {
        lights[5] = greenLight;
      }
      if (this.props.socketio.data[1].standingWaveProtection === 1) {//驻波保护
        lights[6] = redLight;
      }
      else if (this.props.socketio.data[1].standingWaveProtection === 0) {
        lights[6] = greenLight;
      }
      if (this.props.socketio.data[1].communicationState === 1) {//通信状态
        lights[7] = redLight;
      }
      else if (this.props.socketio.data[1].communicationState === 0) {
        lights[7] = greenLight;
      }
    }
        ...
        <Draggable handle="strong" {...dragHandlers}>
          <div hidden={this.props.hidden} className={styles.Single_Extension}>
            ...
            <div className={styles.Three_State}>
              ...
              <br />
              <span className={styles.Three_State_LightSpan}>过荷</span>
            </div>
            <br />
